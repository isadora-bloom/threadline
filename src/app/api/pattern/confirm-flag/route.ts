...
export async function POST(req: NextRequest) {
  ...
  let body: { flagId?: string; note?: string; status?: 'worth_investigating' | 'confirmed' }
  try {
    body = await req.json()
  } catch {
  ...
  const { flagId, note, status = 'worth_investigating' } = body
  if (!flagId) {
    return NextResponse.json({ error: 'flagId is required' }, { status: 400 })
  }

  if (status !== 'worth_investigating' && status !== 'confirmed') {
    return NextResponse.json({ error: 'status must be worth_investigating or confirmed' }, { status: 400 })
  }
  ...
  await supabase.from('review_actions').insert({
    ...
    note: `Pattern flag marked as ${status}: ${flagId}${note ? ` — ${note}` : ''}`,
  })
